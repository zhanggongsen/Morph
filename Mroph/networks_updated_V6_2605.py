...
def define_MorphNet(input_nc, output_nc, ngf, netG, norm='group', use_dropout=False, init_type='normal', init_gain=0.02,
             gpu_ids=[], ifattention=False, attention_form='AG', input_size=(64, 192, 288),
             final_activation=None, return_components=True, anatomy_in_channels=3):
    # net = None
    ...
    morph_kwargs = dict(input_size=input_size, final_activation=final_activation,
                        return_components=return_components, anatomy_in_channels=anatomy_in_channels)

    if netG == 'UNet3D':
        net = UNet3D(input_nc, output_nc, f_maps=[16, 32, 64, 128], layer_order=layer_order,
                     num_groups=8,
                     num_levels=4, conv_padding=1, attention=False, **morph_kwargs)

    elif netG == "Swin_MTL":
        net = MorphNet(input_nc, output_nc, f_maps=[16, 32, 64, 128],
        ...
        net = MorphNet(input_nc, output_nc, f_maps=[16, 32, 64, 128],
                             ...
                             attention_form=attention_form, **morph_kwargs)
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        if attention:
            if attention_form == 'AG':
                self.attention_gate = AG_Attention_block(F_g=out_channels, F_l=out_channels,
                                                         F_int=max(int(out_channels / 2), 1),
                                                         num_groups=num_groups)
                ...
                raise ValueError("Unsupported attention_form '%s'. Expected 'AG' or 'CBAM'." % attention_form)
        ...
        self.basic_module = basic_module(decoder_in_channels, out_channels,
                                         encoder=False,
                                         kernel_size=conv_kernel_size,
                                         order=conv_layer_order,
                                         num_groups=num_groups,
    ...
    def forward(self, encoder_features, x):
        ...
        if self.if_attention:
            if self.attention_form == 'AG':
                encoder_features = self.attention_gate(g=encoder_features, x=x)
            elif self.attention_form == 'CBAM':
                encoder_features = self.CBAM_attention_1(encoder_features) + self.CBAM_attention_2(x)
...
class CBAM_Attention_block(nn.Module):
    def __init__(self, channel, reduction=16, spatial_kernel=7):
        ...
        # shared MLP over the channel dimension
    ...
    def forward(self, x):
        ...
        max_out = self.mlp(self.max_pool(x).view(b, c)).view(b, c, 1, 1, 1)
        avg_out = self.mlp(self.avg_pool(x).view(b, c)).view(b, c, 1, 1, 1)
...
def _valid_num_groups(num_channels, requested_groups):
...
def _make_fusion_block(in_channels, out_channels, layer_order='gcr', num_groups=8):
    modules = [nn.Conv3d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=False)]
    if 'g' in layer_order:
        modules.append(nn.GroupNorm(num_groups=_valid_num_groups(out_channels, num_groups),
                                    num_channels=out_channels))
    elif 'b' in layer_order:
...
class Abstract3DUNet(nn.Module):
    def __init__(self, in_channels, out_channels, basic_module, f_maps=16, layer_order='gcr',
        ...
        assert len(f_maps) >= 4, "MorphNet requires at least 4 feature levels, e.g. [16, 32, 64, 128]"

        input_size = kwargs.pop('input_size', (64, 192, 288))
        self.return_components = kwargs.pop('return_components', False)
        final_activation = kwargs.pop('final_activation', None)
        self.anatomy_in_channels = kwargs.pop('anatomy_in_channels', 3)
        ...
        self.encoders_1 = create_encoders(1, f_maps[0:2], basic_module, conv_kernel_size, conv_padding,
        ...
        # Branch 4
        self.encoders_4 = create_encoders(self.anatomy_in_channels, f_maps[0:4], basic_module,
                                          conv_kernel_size, conv_padding, layer_order, num_groups,
                                          pool_kernel_size)

        # Encoder-side CBAMs described by MorphNet: two surrogate sub-encoders, final shared
        # surrogate encoder, and final anatomical-prior encoder.
        self.cbam_encoder_1 = nn.ModuleList([CBAM_Attention_block(ch, reduction=16, spatial_kernel=7)
                                             for ch in f_maps[0:2]])
        self.cbam_encoder_2 = nn.ModuleList([CBAM_Attention_block(ch, reduction=16, spatial_kernel=7)
                                             for ch in f_maps[0:2]])
        ...
        self.skip_fusion_sub = _make_fusion_block(16 + 32 + f_maps[0], 16, layer_order, num_groups)

        self.decoders_0 = create_decoders([32, 64, 384], basic_module, conv_kernel_size, conv_padding,
                                          layer_order, num_groups, upsample=True,
                                          attention=attention, attention_form=attention_form)
        self.decoders_1 = create_decoders(f_maps[0:2], basic_module, conv_kernel_size, conv_padding,
                                          layer_order, num_groups, upsample=True,
                                          attention=attention, attention_form=attention_form)
        self.decoders_2 = create_decoders(f_maps[0:2], basic_module, conv_kernel_size, conv_padding,
                                          layer_order, num_groups, upsample=True,
                                          attention=attention, attention_form=attention_form)
        self.decoders_3 = create_decoders(f_maps[0:2], basic_module, conv_kernel_size, conv_padding,
                                          layer_order, num_groups, upsample=True,
                                          attention=attention, attention_form=attention_form)
        ...
        if final_activation is None or final_activation == 'none':
            ...
            raise ValueError("Unsupported final_activation '%s'. Use None, 'none', 'sigmoid', or 'tanh'." %
                             final_activation)

    def _swin_tokens_to_feature_maps(self, swin_outputs, x):
        encoders_0_features = []
        encoders_0_features_reverse = list(reversed(swin_outputs))
        decay = pow(2, len(encoders_0_features_reverse) - 1)
        D, H, W = int(x.shape[2] / decay), int(x.shape[3] / decay), int(x.shape[4] / decay)
        for feature in encoders_0_features_reverse:
            encoders_0_features.append(
                einops.rearrange(feature, 'b (d h w) c -> b c d h w', d=D, h=H, w=W))
            D, H, W = int(D * 2), int(H * 2), int(W * 2)
        return encoders_0_features

    @staticmethod
    def _fuse_skip(primary, auxiliary, fusion_block, *extra_auxiliaries):
        tensors = [primary, _resize_like(auxiliary, primary)]
        tensors.extend(_resize_like(auxiliary_extra, primary) for auxiliary_extra in extra_auxiliaries)
        return fusion_block(torch.cat(tensors, dim=1))

    def _decode_direction(self, decoders, x_decoder_main, encoder_sub_skip):
        x_decoder_sub = x_decoder_main
        for decoder, encoder_features in zip(decoders, [encoder_sub_skip]):
            x_decoder_sub = decoder(encoder_features, x_decoder_sub)
        return self.final_conv(x_decoder_sub)

    def forward(self, x, y, z, anatomy=None):

        encoders_0_features_origin = self.encoders_0(x)
        encoders_0_features = self._swin_tokens_to_feature_maps(encoders_0_features_origin, x)
        x_0 = encoders_0_features[0]

        encoders_1_features = []
        for i, encoder_1 in enumerate(self.encoders_1):
            y = encoder_1(y)
            y = self.cbam_encoder_1[i](y)
            encoders_1_features.append(y)

        encoders_2_features = []
        for i, encoder_2 in enumerate(self.encoders_2):
            z = encoder_2(z)
            z = self.cbam_encoder_2[i](z)
            encoders_2_features.append(z)

        yz_full_skip = torch.cat((encoders_1_features[0], encoders_2_features[0]), dim=1)
        y_z = torch.cat((encoders_1_features[-1], encoders_2_features[-1]), dim=1)

        encoders_3_features = []
        for encoder_3 in self.encoders_3:
            y_z = encoder_3(y_z)
            encoders_3_features.append(y_z)
        y_z = self.cbam_encoder_3(y_z)
        encoders_3_features[-1] = y_z

        if anatomy is None:
            anatomy = x.new_zeros((x.shape[0], self.anatomy_in_channels, x.shape[2], x.shape[3], x.shape[4]))

        encoders_4_features = []
        for encoder_4 in self.encoders_4:
            anatomy = encoder_4(anatomy)
            encoders_4_features.append(anatomy)
        anatomy = self.cbam_encoder_4(anatomy)
        encoders_4_features[-1] = anatomy

        x_decoder_main = torch.cat((x_0, _resize_like(y_z, x_0), _resize_like(anatomy, x_0)), dim=1)

        # Fuse Swin skip features with corresponding surrogate and anatomical-prior features.
        encoders_0_features_main = [
            self._fuse_skip(encoders_0_features[1], encoders_3_features[1], self.skip_fusion_main[0],
                            encoders_4_features[2]),
            self._fuse_skip(encoders_0_features[2], encoders_3_features[0], self.skip_fusion_main[1],
                            encoders_4_features[1]),
        ]
        encoder_sub_skip = self._fuse_skip(encoders_0_features[3], yz_full_skip, self.skip_fusion_sub,
                                           encoders_4_features[0])
        ...
        # Direction-specific sub-decoders for LR/AP/SI components.
        x_decoder_sub_1 = self._decode_direction(self.decoders_1, x_decoder_main, encoder_sub_skip)
        x_decoder_sub_2 = self._decode_direction(self.decoders_2, x_decoder_main, encoder_sub_skip)
        x_decoder_sub_3 = self._decode_direction(self.decoders_3, x_decoder_main, encoder_sub_skip)
        ...
        return torch.cat((x_decoder_sub_1, x_decoder_sub_2, x_decoder_sub_3), dim=1)
...
class MorphNet(Abstract3DUNet):
    def __init__(self, in_channels, out_channels, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=5, conv_padding=1, attention=False, attention_form='AG',
                 **kwargs):
        super(MorphNet, self).__init__(in_channels=in_channels,
                                             out_channels=out_channels,
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def _create_padding_mask(
        ...
        # Mark padded voxels so attention cannot mix real and padded tokens.
        # The original implementation indexed the wrong spatial axes when height/width
        # padding was larger than depth, which could fail for smaller test volumes.
        id_map = torch.zeros((1, input_size[0], input_size[1], input_size[2], 1))
        cnt = 1
        for p in range(1, pad_size[0] + 1):
            id_map[:, -p, :, :, :] = cnt
            cnt += 1
        for p in range(1, pad_size[1] + 1):
            id_map[:, :, -p, :, :] = cnt
            cnt += 1
        for p in range(1, pad_size[2] + 1):
            id_map[:, :, :, -p, :] = cnt
            cnt += 1
        id_map = id_map.view(1, -1, 1).contiguous()
